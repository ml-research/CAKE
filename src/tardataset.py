# Original source: https://raw.githubusercontent.com/jotaf98/simple-tar-dataset/master/tardataset.py
...
class TarDataset(Dataset):
    ...
    For this project, modified by: Steven Braun
    """
    ...
    def __init__(
        ...
        archive,
        transform=to_tensor,
        extensions=(".png", ".jpg", ".jpeg", ".csv", ".pt", ".np", ".npz"),
        eps_min=0.0,
        eps_max=1e10,
        is_valid_file=None,
        ...
        self.filter_samples_by_eps(eps_min=eps_min, eps_max=eps_max)

        self.transform = transform

    def filter_samples_by_eps(self, eps_min, eps_max):
        """
        Filter samples by epsilon value.

        Args:
          eps_min: Epsilon min value.
          eps_max: Epsilon max value.
        """
        filtered = []
        for name in self.samples:
            eps = float(name.split("/")[0].split("_")[1])
    ...
    def __getitem__(self, index):
        ...
        data = self.get_file(sample).read()
        array_file = BytesIO()
        array_file.write(data)
        array_file.seek(0)
