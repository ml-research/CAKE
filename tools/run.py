...
def generate_run_base_dir(result_dir: str, timestamp: int = None, tag: str = None, sub_dirs: List[str] = None) -> str:
    """
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=f"default-{int(time.time_ns())}",
    )
    parser.add_argument(
        "-d",
        "--detach",
...
def get_docker_volumes():
    ...
    mapping = {
        ...
        f"{data_dir}": f"{home_dir}/data/:z",
        f"{models_dir}": f"{home_dir}/models/:z",
        f"{results_dir}": f"{home_dir}/results/:z",
    }
...
def get_docker_options():
    # Default name
    name = f"default-{int(time.time_ns())}"

    args_command = " ".join(ARGS.command)

    # Extract group tag
    match = re.search(r"env\.group_tag=([^\s]+)", args_command)
    if match:
        group_tag = match.group(1)
        name = group_tag

    match = re.search(r"env\.tag=([^\s]+)", args_command)
